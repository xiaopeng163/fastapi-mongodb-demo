...
from app.database import retrieve_task
from app.database import retrieve_tasks
from app.database import update_task
from app.models.base import ResponseModel
from app.models.schemas import TaskModel
from app.models.schemas import TaskUpdateModel
...
async def _update_label_data(id: str, req: TaskUpdateModel):
    ...
    return HTTPException(
        status_code=status.HTTP_400_BAD_REQUEST, detail='failed')
...
async def _delete_task_data(id: str):
    ...
    if deleted_task:
        return ResponseModel(
            f'Task with ID: {id} removed', 'Task deleted successfully')
    return HTTPException(
        status_code=status.HTTP_400_BAD_REQUEST, detail='failed')
